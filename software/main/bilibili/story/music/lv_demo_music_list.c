...
lv_obj_t * vocat_lv_demo_music_list_create(lv_obj_t * parent)
{
    ...
    uint32_t track_id;
    for(track_id = 0; vocat_lv_demo_music_get_title(track_id); track_id++) {
...
static lv_obj_t * add_list_button(lv_obj_t * parent, uint32_t track_id)
{
    uint32_t t = vocat_lv_demo_music_get_track_length(track_id);
    char time[32];
    lv_snprintf(time, sizeof(time), "%"LV_PRIu32":%02"LV_PRIu32, t / 60, t % 60);
    const char * title = vocat_lv_demo_music_get_title(track_id);
    const char * artist = vocat_lv_demo_music_get_artist(track_id);
...
static void btn_click_event_cb(lv_event_t * e)
{
    ...
    if(page_root != NULL) {
        lv_obj_set_style_anim_duration(page_root, 180, 0);
